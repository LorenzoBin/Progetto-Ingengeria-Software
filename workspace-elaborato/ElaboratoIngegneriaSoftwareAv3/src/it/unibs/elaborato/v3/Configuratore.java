	...
	public void aggiornaStatiEInviaNotifiche() {
		...
		for (Proposta p : bachecaReale.getTutte()) {
			...
			if (stato.equals("Aperta")) {
				...
				// Se oggi ha superato la data di scadenza (ovvero è scoccata la mezzanotte)
				if (scadenza != null && oggi.isAfter(scadenza)) {
					
					if (p.getIscritti().size() == p.getMaxPartecipanti()) {
						p.cambiaStato("Confermata");
						
						// --- CORREZIONE PUNTO 1: Promemoria completo ---
						String promemoria = String.format(
							"L'iniziativa '%s' è CONFERMATA! Promemoria -> Data: %s, Ora: %s, Luogo: %s, Quota: %s.", 
							p.getValore("Titolo"), 
							p.getValore("Data"), 
							p.getValore("Ora"), 
							p.getValore("Luogo"), 
						...
						inviaNotificaA(p.getIscritti(), "L'iniziativa '" + p.getValore("Titolo") + "' è stata ANNULLATA per mancato raggiungimento del numero di iscritti.");
					}
				}
			} 
